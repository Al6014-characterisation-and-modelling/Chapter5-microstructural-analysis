...
size = 'large' %here I choose between small or large EBSD
...
equivDiameters = 2 * sqrt(abs(cleaned_grains.area) ./ pi);


% Plot grains colored by equivalent diameter
figure;
plot(cleaned_grains, equivDiameters);
cb = colorbar;
...
aspectRatio = cleaned_grains.aspectRatio % Major axis length

% Plot grains with aspect ratio as color
figure;
plot(cleaned_grains, aspectRatio);
...
%% Save equivalent diamters and aspect ratios into a csv file

if strcmp(export_csv, 'yes')
    T = table(cleaned_grains.id, equivDiameters, aspectRatio, ...
